...
BOT_TOKEN = "7399940837:AAGFVUMyG-bZmSygSj-5wP_T0gTuauSngAg"  # ضع توكن البوت هنا
...
def find_main_script(bot_dir):
    # إعطاء الأولوية للملفات الأساسية المتعارف عليها
    for root, dirs, files in os.walk(bot_dir):
        for common_name in ["main.py", "bot.py", "app.py", "run.py", "index.py"]:
            if common_name in files:
                return os.path.join(root, common_name)
    # كحل بديل، البحث عن أي ملف بايثون
    for root, dirs, files in os.walk(bot_dir):
        for file in files:
            if file.endswith(".py"):
    ...
    if req_file:
        ...
        try:
            with open(script_path, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read()
            imports = set()
            stdlibs = {"os", "sys", "re", "json", "time", "asyncio", "math", "random", "datetime", "shutil", "logging", "typing", "subprocess", "base64", "urllib", "sqlite3", "threading", "collections", "itertools", "pathlib", "hashlib"}
            for line in content.split("\n"):
                match = re.match(r'^\s*(?:import|from)\s+([a-zA-Z0-9_]+)', line)
                if match:
                    mod = match.group(1)
                    if mod not in sys.builtin_module_names and mod not in stdlibs:
                        imports.add(mod)
            if imports:
                subprocess.run([sys.executable, "-m", "pip", "install"] + list(imports))
        except:
            pass
...
async def start_bot_process(user_id, slot, bot_dir, log_mode="a"):
    process_key = f"{user_id}_{slot}"
    script_path = find_main_script(bot_dir)
    
    if not script_path:
        return False, "❌ لم يتم العثور على ملف بايثون رئيسي للتشغيل (مثل main.py أو bot.py)."
        
    script_name = os.path.basename(script_path)
    script_dir = os.path.dirname(script_path)
    log_path = f"hostings/{user_id}/slot_{slot}/log.txt"
    
    # تحميل المكاتب في مسار جانبي لمنع تجميد بوت الاستضافة
    await asyncio.to_thread(auto_install_requirements, bot_dir, script_path)
    
    # إيقاف النسخة القديمة إن وجدت
    if process_key in running_bots:
        try: running_bots[process_key].terminate()
        except: pass
        del running_bots[process_key]
        
    try:
        # استخدام sys.executable لضمان التوافق التام
        # استخدام -u لمنع التخزين المؤقت للأخطاء (Unbuffered)
        p = subprocess.Popen(
            [sys.executable, "-u", script_name],
            cwd=script_dir,
            stdin=subprocess.PIPE,
            stdout=open(log_path, log_mode, encoding="utf-8"),
            stderr=subprocess.STDOUT
        )
        running_bots[process_key] = p
    except Exception as e:
        return False, f"❌ فشل تشغيل العملية الأساسية: {e}"
        
    # الانتظار للتحقق مما إذا كان البوت سيتوقف فوراً بسبب خطأ برمجي
    await asyncio.sleep(2.5)
    if p.poll() is not None:
        del running_bots[process_key]
        try:
            with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                log_err = "".join(f.readlines()[-25:])
        except:
            log_err = "لا يوجد تفاصيل للخطأ في السجل."
        return False, f"❌ البوت توقف فوراً بعد التشغيل.\nيوجد خطأ في الكود المرفوع:\n```\n{log_err[-3000:]}\n```"
        
    return True, f"✅ تم تشغيل البوت بنجاح. (رقم التنصيب: {slot})"
...
async def handle_texts(client: Client, message: Message):
    ...
    if step == "WAITING_INPUT":
        ...
        # التعديل الهام: أخذ النص كما هو لحماية جلسات بايروجرام وتليثون
        raw_input = text.strip()
        
        if process_key in running_bots and running_bots[process_key].poll() is None:
            try:
                running_bots[process_key].stdin.write(f"{raw_input}\n".encode('utf-8'))
                running_bots[process_key].stdin.flush()
                await message.reply("✅ تم الإدخال وإرسال البيانات للبوت بنجاح.")
            except Exception as e:
    ...
    # ================= 4. أزرار قسم الإدارة (البوت والملفات) =================
        ...
        if text == "سجل البوت":
            ...
            if os.path.exists(log_path):
                ...
                return await message.reply(f"**سجل التنصيب (آخر السطور):**\n```\n{log_text[-4000:]}\n```")
            else:
            ...
            success, result_text = await start_bot_process(target_id, slot, bot_dir, log_mode="a")
            return await msg.edit_text(result_text)

        elif text == "🔄 إعادة تشغيل":
            msg = await message.reply("⏳ جاري إعادة التشغيل الشاملة...")
            success, result_text = await start_bot_process(target_id, slot, bot_dir, log_mode="a")
            return await msg.edit_text(result_text)
            ...
            if process_key in running_bots and running_bots[process_key].poll() is None:
                ...
                return await message.reply("أدخل القيمة الآن بدقة (لا تقم بإضافة مسافات إضافية):")
            else:
...
async def execute_admin_user_action(message, action, target_id, slot):
    ...
    if action == "إدارة تنصيب عضو":
        ...
        msg = await message.reply("⏳ جاري تشغيل التنصيب للعضو...")
        success, result_text = await start_bot_process(target_id, slot, f"{user_dir}/bot", log_mode="a")
...
async def handle_docs(client: Client, message: Message):
                        ...
                        if os.path.isdir(user_path):
                            for slot_dir in os.listdir(user_path):
                                if slot_dir.startswith("slot_"):
                                    ...
                                    bot_dir = f"{user_path}/{slot_dir}/bot"
                                    # تشغيل صامت للنسخ الاحتياطية
                                    script_path = find_main_script(bot_dir)
                                    if script_path:
                                        p = subprocess.Popen([sys.executable, "-u", os.path.basename(script_path)], cwd=os.path.dirname(script_path), stdin=subprocess.PIPE, stdout=open(f"{user_path}/{slot_dir}/log.txt", "a", encoding="utf-8"), stderr=subprocess.STDOUT)
                                        running_bots[f"{uid}_{slot_num}"] = p
                                        restarted_count += 1
                                        
                user_states[user_id]["step"] = None
                await msg.edit_text(f"✅ تم استعادة النسخة بنجاح!\n🤖 تم تشغيل {restarted_count} بوت تلقائياً.")
            except Exception as e:
    ...
    if step == "WAITING_REPLACE_FILE":
        ...
        if os.path.exists(target_path) and os.path.isfile(target_path):
            ...
            state["step"] = None
            return await message.reply(f"❌ خطأ: لا يوجد ملف باسم `{file_name}` في المجلد لتبديله.")

    # --- تنصيب بوت جديد ---
    elif step == "WAITING_FOR_ZIP":
        if not (file_name.endswith(".zip") or file_name.endswith(".py")):
            return await message.reply("❌ يتم قبول ملفات `.zip` أو `.py` فقط.")

        slot = state.get("slot")
        msg = await message.reply("⏳ جاري سحب الملفات وفك الضغط...")
        ...
        await msg.edit_text("⏳ جاري التجهيز والتشغيل والبحث عن أخطاء في الكود...")
        
        # استخدام دالة التشغيل الموحدة 
        success, result_text = await start_bot_process(user_id, slot, bot_dir, log_mode="w")
        
        # حتى لو فشل التشغيل بسبب خطأ في كود البوت المرفوع، سنبقي الملفات لكي يقوم بتعديلها من "إدارة الملفات"
        user_states[user_id]["step"] = None
        await msg.edit_text(result_text, reply_markup=main_menu(user_id))
